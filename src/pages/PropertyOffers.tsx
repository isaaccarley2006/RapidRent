...
import { ArrowLeft, Building, MapPin, Users, TrendingUp, Clock, DollarSign, Star, Eye } from 'lucide-react';
...
export default function PropertyOffers() {
  ...
  const fetchPropertyAndOffers = async () => {
    ...
    try {
      ...
      const {
        data: propertyData,
        error: propertyError
      } = await supabase.from('properties').select('*').eq('id', propertyId).eq('landlord_id', user.id).single();
      ...
      const {
        data: offersData,
        error: offersError
      } = await supabase.from('offers').select('*').eq('property_id', propertyId).order('created_at', {
        ascending: false
      });
      if (offersError) throw offersError;

      // Manually fetch profiles for each offer
      const offersWithProfiles = await Promise.all((offersData || []).map(async offer => {
        const {
          data: profileData
  ...
  const setupRealtimeSubscription = () => {
    ...
    const channel = supabase.channel(`property-${propertyId}-offers`).on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'offers',
  ...
  const getPropertyInsights = () => {
    ...
    const avgTenantScore = offers.filter(o => o.profiles).reduce((sum, offer) => {
      // Calculate score for each tenant (simplified version)
      const verifications = [offer.profiles?.identity_verified, offer.profiles?.employment_verified, offer.profiles?.income_verified, offer.profiles?.credit_verified, offer.profiles?.references_verified, offer.profiles?.bank_verified].filter(Boolean).length;
  ...
  return <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-card rounded-xl border border-border p-6">
        <div className="flex items-start gap-6">
          {property.images && property.images.length > 0 ? <img src={property.images[0]} alt={property.title} className="w-32 h-24 object-cover rounded-lg" onError={e => {
          ...
          <div className="flex-1">
            ...
            {property.location && <div className="flex items-center gap-2 text-muted-foreground mb-2">
                <MapPin className="w-4 h-4" />
                <span>{property.location}</span>
              </div>}
            <div className="flex items-center gap-4">
              {property.price && <span className="text-xl font-semibold text-primary">
      ...
      {insights && <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-lg">5.6 days</p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {offers.length === 0 ? "No offers received yet for this property" : `${offers.length} offer${offers.length !== 1 ? 's' : ''} received`}
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          }} updating={updating} onUpdateStatus={handleUpdateStatus} onSelectOffer={() => {}} />)}
            </div>}
        </CardContent>
      </Card>
    </div>;
