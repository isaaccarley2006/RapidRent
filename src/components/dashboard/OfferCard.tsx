...
export const OfferCard: React.FC<OfferCardProps> = ({
  ...
  const calculateTenantScore = (): number => {
    ...
    const verifications = [offer.profiles.identity_verified, offer.profiles.employment_verified, offer.profiles.income_verified, offer.profiles.credit_verified, offer.profiles.references_verified, offer.profiles.bank_verified];
    ...
    if (offer.profiles.credit_score) {
      ...
      if (creditScore >= 800) score += 30;else if (creditScore >= 700) score += 25;else if (creditScore >= 600) score += 20;else if (creditScore >= 500) score += 15;else score += 10;
    }
    maxScore += 30;
    ...
    if (offer.profiles.annual_income && offer.properties?.price) {
      ...
      if (rentToIncomeRatio <= 25) score += 20;else if (rentToIncomeRatio <= 30) score += 15;else if (rentToIncomeRatio <= 35) score += 10;else score += 5;
    }
    maxScore += 20;

    // Employment status (10% of total score)
    if (offer.profiles.employment_status) {
      if (offer.profiles.employment_status === 'employed') score += 10;else if (offer.profiles.employment_status === 'self-employed') score += 8;else if (offer.profiles.employment_status === 'student') score += 6;else score += 3;
    }
    maxScore += 10;
  ...
  return <Card className="relative overflow-hidden border border-border hover:shadow-md transition-shadow animate-fade-in">
      {/* Status Badge */}
      ...
      <CardHeader className="pb-3 px-0 py-[25px]">
        <div className="space-y-2">
          <h3 className="font-semibold text-lg text-foreground pr-20">
            {offer.properties?.title || 'Property not found'}
          </h3>
          {offer.properties?.location && <div className="flex items-center gap-1 text-muted-foreground">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            {offer.profiles?.email && <div className="flex items-center gap-2 text-muted-foreground">
                <Mail className="w-3 h-3" />
                <span className="text-sm">{offer.profiles.email}</span>
              </div>}
            {offer.profiles?.phone && <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <div className="p-3 bg-secondary/30 rounded-lg space-y-3">
            ...
            <div className="flex flex-wrap gap-1">
              {offer.profiles?.identity_verified && <Badge variant="secondary" className="text-xs bg-green-100 text-green-700">
                  <CheckCircle className="w-3 h-3 mr-1" />
                  ID
                </Badge>}
              {offer.profiles?.employment_verified && <Badge variant="secondary" className="text-xs bg-blue-100 text-blue-700">
                  <CheckCircle className="w-3 h-3 mr-1" />
                  Employment
                </Badge>}
              {offer.profiles?.income_verified && <Badge variant="secondary" className="text-xs bg-purple-100 text-purple-700">
                  <CheckCircle className="w-3 h-3 mr-1" />
                  Income
                </Badge>}
              {offer.profiles?.credit_verified && <Badge variant="secondary" className="text-xs bg-orange-100 text-orange-700">
                  <CheckCircle className="w-3 h-3 mr-1" />
                  Credit
                </Badge>}
              {offer.profiles?.references_verified && <Badge variant="secondary" className="text-xs bg-teal-100 text-teal-700">
                  <CheckCircle className="w-3 h-3 mr-1" />
                  References
                </Badge>}
              {offer.profiles?.bank_verified && <Badge variant="secondary" className="text-xs bg-indigo-100 text-indigo-700">
                  <CheckCircle className="w-3 h-3 mr-1" />
                  Bank
                </Badge>}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {offer.profiles?.credit_score ? <div className="text-center">
              ...
              {incomeToRentRatio ? <div className="text-center">
                  <div className={`px-2 py-1 rounded text-xs font-medium ${incomeToRentRatio <= 30 ? 'text-green-600 bg-green-50' : incomeToRentRatio <= 40 ? 'text-yellow-600 bg-yellow-50' : 'text-red-600 bg-red-50'}`}>
        ...
        <div className="space-y-3 p-3 bg-secondary/20 rounded-lg">
          ...
          {offer.properties?.price && <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium">
              {offer.preferred_move_in_date ? format(new Date(offer.preferred_move_in_date), 'MMM d, yyyy') : 'Flexible'}
            </span>
          </div>
        </div>
        ...
        {/* Additional Details */}
        <div className="space-y-2">
          {offer.profiles?.employment_status && <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 pt-2">
          <Dialog>
            <DialogTrigger asChild>
              <Button variant="outline" size="sm" onClick={() => onSelectOffer(offer)} className="flex-1 hover-scale">
                <Eye className="w-4 h-4 mr-2" />
                View Details
              </Button>
            </DialogTrigger>
            <OfferDetailsDialog offer={offer} updating={updating} onUpdateStatus={onUpdateStatus} />
          </Dialog>
