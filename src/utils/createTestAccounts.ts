...
export const createTestAccounts = async () => {
  ...
  try {
    ...
    const { data: landlordAuth, error: landlordAuthError } =
    ...
    const { data: tenantAuth, error: tenantAuthError } =
      await supabase.auth.signUp({
    ...
    console.log(
      "Landlord:",
      TEST_ACCOUNTS.landlord.email,
      "/",
      TEST_ACCOUNTS.landlord.password
    );
    console.log(
      "Tenant:",
      TEST_ACCOUNTS.tenant.email,
      "/",
      TEST_ACCOUNTS.tenant.password
    );
  } catch (error) {
